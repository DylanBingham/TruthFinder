...
const container = d3.select("#flesch-reading-ease-chart .chart-svg-container");    
    ...
    const filteredData = data
        .filter(d => +d.flesch_reading_ease > 0 && +d.flesch_reading_ease <= 100)
        .map(d => +d.flesch_reading_ease)
        .filter(d => !isNaN(d));
    filteredData.sort((a, b) => a - b);
    
    // Also, split the data into two groups based on the binary_label
    const dataLabel0 = data
        .filter(d => +d.binary_label === 0 && +d.flesch_reading_ease >= -100)
        .map(d => +d.flesch_reading_ease)
        .filter(d => !isNaN(d));
    const dataLabel1 = data
        .filter(d => +d.binary_label === 1 && +d.flesch_reading_ease >= -100)
        .map(d => +d.flesch_reading_ease)
        .filter(d => !isNaN(d));
    ...
    svg.append("text")
        ...
        .style("text-anchor", "middle")
        .text("Distribution of Flesch Reading Ease Score");
}
